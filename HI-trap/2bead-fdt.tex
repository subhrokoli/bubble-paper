...
separation, and falls off as the latter is increased, so that with
our current detection sensitivity, we do not observe the resonance
effects beyond a surface-surface separation greater than $3a$ - this
increased distance also being smaller than that used in earlier experiments,
...
is also dependent on the stiffness ratio, and increases as the latter
is reduced. For a given medium, the resonance can thus be tuned by
changing the stiffness ratios (as well as the inter-particle separation
and particle diameters). Interestingly, it is obvious that the value
